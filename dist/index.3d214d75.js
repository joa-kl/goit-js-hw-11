...
// })
.then((data)=>renderPicturesList(data)).catch((error)=>{
    console.log(error);
});
searchBtn.addEventListener("click", ()=>{
    fetchPictures().then((pictures)=>renderPicturesList(pictures)).catch(()=>(0, _notiflixDefault.default).Notify.failure("Oops, there is no country with that name"));
});
function onSearch(e) {
    e.preventDefault();
    page = 1;
    ...
    fetchPictures().then(({ data  })=>{
        if (data.totalHits === 0) alertNoContentFound();
        else {
            ...
            if (data.totalHits > perPage) loadMoreButton.classList.remove("is-hidden");
        }
...
function renderPicturesList(data) {
    ...
    const picturesList = hits.map((hit)=>{
        return `
                    <li>
...
function noInfoForSearch() {
    (0, _notiflixDefault.default).Notify.failure("Please specify your search query.");
}
...
},{"notiflix":"5z0Oc","simplelightbox":"9ydBq","simplelightbox/dist/simple-lightbox.min.css":"kaxSc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"5z0Oc":[function(require,module,exports) {
var global = arguments[3];
...
},{}]},["dB9nQ","bB7Pu"], "bB7Pu", "parcelRequired7c6")
